...
1. 可以使用不限数量的无人机组成编队。
2. 无人机保持一定速度飞行，在飞行过程中依次变换出 `"S"`、`"Y"`、`"S"`、`"U"` 四种队形。
3. 变换过程中无人机需保持移动，每种队形需要保持 3 秒，允许起始队形为字母 `"S"`。
4. 在队形保持期间需要体现障碍物避障能力。
5. 仿真场景已固定随机种子，请不要修改 `map_generator` 的代码以及 `normal_hexagon.launch` 文件中该节点的参数。

## 提交内容

1. 源代码，包括 Git 提交记录，因此请不要删除 `.git` 文件夹，并在提交前完成一次 commit。
2. 飞行演示 GIF，固定保存为 `results/demo.gif`。
3. 报告，简述实现思路与演示结果截图，不超过 2 页，固定保存为 `results/report.pdf`。

## 提交方式

1. 完成后请先运行以下命令检查提交完整性：
   ```bash
   bash check_completeness.sh
   ```
2. 确认检查通过后，将最终文件夹压缩为 `姓名-学号.zip`。
3. 提交方式在企业微信群统一通知。
